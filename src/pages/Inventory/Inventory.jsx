...
const Inventory = () => {
    ...
    const filteredInventory = inventory.filter(item => {
        ...
        return (
            (item.make?.toLowerCase()?.includes(searchString) || false) ||
            (item.model?.toLowerCase()?.includes(searchString) || false) ||
            (item.vin?.toLowerCase()?.includes(searchString) || false)
        );
    });
    ...
    const sortedInventory = [...filteredInventory].sort((a, b) => {
        switch (sortBy) {
            case 'newest':
                return (new Date(b.createdAt?.toDate()) - new Date(a.createdAt?.toDate())) || 0;
            case 'oldest':
                return (new Date(a.createdAt?.toDate()) - new Date(b.createdAt?.toDate())) || 0;
            case 'priceHigh':
